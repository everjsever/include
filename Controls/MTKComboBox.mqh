//+------------------------------------------------------------------+
//|                                                     ComboBox.mqh |
//|          Original Copyright 2009-2013, MetaQuotes Software Corp. |
//|                 Modified by MTK Beijing Tech Co., Ltd 2014-12-31 |
//|                                              http://www.mql5.com |
//+------------------------------------------------------------------+
...
#resource "res\\DropOn25.bmp"                 // image file
#resource "res\\DropOff25.bmp"                // image file
//+------------------------------------------------------------------+
//| Class CComboBox                                                  |
...
public:
   ...
   virtual bool      ItemsClear(void)                                                 { m_edit.Text(""); return(m_list.ItemsClear());                 }
   //--- data
   ...
   long              Current(void) { return (m_list.Current()); }
   //--- state
   ...
   //--- other option
   void              SetReadOnly(const bool flags);
...
protected:
   ...
   virtual bool      OnDoubleClickEdit(void);
   virtual bool      OnClickButton(void);
   virtual bool      OnClickThis(void);
   virtual bool      OnDoubleClickThis(void);
   virtual bool      OnDoubleClickButton(void);
...
EVENT_MAP_BEGIN(CComboBox)
   ...
   ON_EVENT(ON_DBL_CLICK,m_edit,OnDoubleClickEdit)
   ON_EVENT(ON_CLICK,this,OnClickThis)
   ON_EVENT(ON_DBL_CLICK,this,OnDoubleClickThis)
   ON_EVENT(ON_CLICK,m_drop,OnClickButton)
   ON_EVENT(ON_DBL_CLICK,m_drop,OnDoubleClickButton)
...
void CComboBox::SetReadOnly(const bool flags)
{
   m_edit.ReadOnly(flags);
}
void CComboBox::SetFont(string name)
{
   m_edit.Font(name);
   m_drop.Font(name);
...
//--- call method of the parent class
   if(!CWndContainer::Create(chart,name,subwin,x1,y1,x2+10,y2))
      return(false);
//--- create dependent controls
   if(!CreateEdit())
      return(false);
...
bool CComboBox::CreateEdit(void)
  {
//--- create
   if(!m_edit.Create(m_chart_id,m_name+"Edit",m_subwin,0,0,Width()-(CONTROLS_BUTTON_SIZE+CONTROLS_COMBO_BUTTON_X_OFF),Height()))
      return(false);
   if(!m_edit.Text(""))
      return(false);
...
//--- create
   if(!m_drop.Create(m_chart_id,m_name+"Drop",m_subwin,x1-7,y1,x2,y2))
      return(false);
   if(!m_drop.BmpNames("::res\\DropOff25.bmp","::res\\DropOn25.bmp"))
      return(false);
   if(!Add(m_drop))
      return(false);
   m_drop.Locking(true);
...
bool CComboBox::OnDoubleClickEdit(void)
  {
//--- change button state
   if(!m_drop.Pressed(!m_drop.Pressed()))
      return(false);
//--- call the click on button handler
  ...
  bool CComboBox::OnDoubleClickButton(void)
  {
      if(!m_drop.Pressed(!m_drop.Pressed()))
         return(false);
      return(OnClickButton());
  ...
  bool CComboBox::OnClickThis(void)
  {
      if(!m_drop.Pressed(!m_drop.Pressed()))
         return(false);
      return(OnClickButton());
  ...
  bool CComboBox::OnDoubleClickThis(void)
  {
      if(!m_drop.Pressed(!m_drop.Pressed()))
         return(false);
      return(OnClickButton());
