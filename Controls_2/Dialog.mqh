...
private:
   ...
   color             CLIENT_BG_COLOR;
   color             CLIENT_BORDER_COLOR;
...
public:
                    ...
                    ~CDialog(void);
   //--- Colors
   void              setColor_CaptionBG(const color _clr) { CAPTION_BG_COLOR = _clr; }
   void              setColor_Caption(const color _clr) { CAPTION_COLOR = _clr; }
   void              setColor_ClientBG(const color _clr) { CLIENT_BG_COLOR = _clr; }
   void              ColorUpdate();
   void              setColor_ClientBorder(const color _clr) { CLIENT_BORDER_COLOR = _clr; }
   //--- create
...
CDialog::CDialog(void) : m_panel_flag(false),
                         m_minimized(false)
  {
  CLIENT_BG_COLOR = CONTROLS_DIALOG_COLOR_CLIENT_BG;
  CLIENT_BORDER_COLOR = CONTROLS_DIALOG_COLOR_BG;
  CAPTION_BG_COLOR = CONTROLS_DIALOG_COLOR_CAPTION_TEXT;
  CAPTION_COLOR = CONTROLS_DIALOG_COLOR_CAPTION_TEXT;
  }
//+------------------------------------------------------------------+
//| Destructor                                                       |
//+------------------------------------------------------------------+
...
/*void CDialog::Lock(void){
   if(!coverCreated) {
      if(!runCover.Create(m_chart_id, m_name+"dlgRunCover", m_subwin, 0, 0, Width(), Height())) return;
      runCover.BmpName("::res\\cover.bmp");
      if(!CWndContainer::Add(runCover)) return;
      coverCreated = true;      
      runCover.BringToTop();
   }
   runCover.Show();
...
void CDialog::UnLock(void) {
   if(coverCreated) runCover.Hide();
   locked = false;
   BringToTop();
}*/
//+------------------------------------------------------------------+
...
void CDialog::ColorUpdate(void){
   m_white_border.ColorBackground(CLIENT_BORDER_COLOR);
   m_background.ColorBackground(CLIENT_BORDER_COLOR);
   color border=(m_panel_flag) ? CLIENT_BORDER_COLOR : CONTROLS_DIALOG_COLOR_BORDER_DARK;
   m_background.ColorBorder(border);
   m_caption.Color(CAPTION_COLOR);
   m_caption.ColorBackground(CAPTION_BG_COLOR);
   m_caption.ColorBorder(CAPTION_BG_COLOR);
   m_client_area.ColorBackground(CLIENT_BG_COLOR);
   m_client_area.ColorBorder(CLIENT_BORDER_COLOR);
}
//+------------------------------------------------------------------+
//| Create "white border"                                            |
//+------------------------------------------------------------------+
...
//--- create
   if(!m_white_border.Create(m_chart_id,m_name+"Border",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_white_border.ColorBackground(CLIENT_BORDER_COLOR))
      return(false);
...
//--- create
   ...
   if(!m_background.Create(m_chart_id,m_name+"Back",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_background.ColorBackground(CLIENT_BORDER_COLOR))
      return(false);
   color border=(m_panel_flag) ? CLIENT_BORDER_COLOR : CONTROLS_DIALOG_COLOR_BORDER_DARK;
...
//--- create
   if(!m_caption.Create(m_chart_id,m_name+"Caption",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_caption.Color(CAPTION_COLOR))
      return(false);
   if(!m_caption.ColorBackground(CAPTION_BG_COLOR))
      return(false);
   if(!m_caption.ColorBorder(CAPTION_BG_COLOR))
      return(false);
...
//--- create
   if(!m_client_area.Create(m_chart_id,m_name+"Client",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_client_area.ColorBackground(CLIENT_BG_COLOR))
      return(false);
   if(!m_client_area.ColorBorder(CLIENT_BORDER_COLOR))
      return(false);
